  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/80 p-4" onClick={onClose}>
      <form
        ...
        aria-modal="true"
        className="my-auto flex w-full max-w-lg flex-col overflow-hidden rounded-3xl border border-white/10 bg-[#111115] text-white shadow-[0_30px_100px_-30px_rgba(0,0,0,0.95)]"
      ...
      >
        ...
        <div className="space-y-6 px-5 py-5 sm:px-6 sm:py-6">
          <section className="flex items-center gap-4 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4">
            <Avatar src={preview} username={username || user.username} size={72} />
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-white">Profile photo</p>
              <p className="mt-1 text-xs leading-5 text-white/40">A clear portrait makes it easier to recognize your work.</p>
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => inputRef.current?.click()}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.055] px-2.5 py-1.5 text-[11px] font-semibold text-white/70 transition hover:bg-white/[0.1] hover:text-white"
                >
                ...
                {preview && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-red-300 transition hover:bg-red-500/10 hover:text-red-200"
                  >
          ...
          <div>
            ...
            <textarea
              ...
              className="mt-2 min-h-28 w-full resize-none rounded-xl border border-white/10 bg-white/[0.035] px-3.5 py-3 text-sm leading-6 text-white outline-none placeholder:text-white/25 focus:border-red-400/70 focus:ring-2 focus:ring-red-500/15"
            />
          </div>
